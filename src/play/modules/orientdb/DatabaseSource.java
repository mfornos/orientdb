...
public class DatabaseSource implements BeanSource {
	...
	public DatabaseSource(int conf) {
		this.documentDB = isEnabled(conf, ODBPlugin.OIV_DOCUMENT_DB) ? ODB
				.openDocumentDB() : null;
		this.objectDB = isEnabled(conf, ODBPlugin.OIV_OBJECT_DB) ? ODB
				.openObjectDB() : null;
		this.graphDB = isEnabled(conf, ODBPlugin.OIV_GRAPH_DB) ? ODB
				.openGraphDB() : null;
	}
